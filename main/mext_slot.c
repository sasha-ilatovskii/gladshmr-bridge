...
// -----------------------------------------------------------------------
typedef enum { ST_HEADER, ST_PAYLOAD } parse_state_t;
...
static void process_byte(uint8_t itf, int slot_idx, parser_t *par, uint8_t b) {
  ...
  case ST_HEADER:
    ...
    if (par->plen == 0) {
      if (slot->type == MONOME_SLOT_GRID) {
        ...
        default:
          ESP_LOGW(TAG, "[CDC%d] unknown 0-payload grid cmd 0x%02x", itf,
                   par->hdr);
          break;
        }
        ...
        case MEXT_SYSTEM_GET_GRIDSZ:
          ...
          break;
        default:
          ESP_LOGW(TAG, "[CDC%d] unknown 0-payload arc cmd 0x%02x", itf,
                   par->hdr);
        }
      }
    } else {
  ...
  case ST_PAYLOAD:
    ...
    if (par->pidx >= par->plen) {
      ...
      ESP_LOGD(TAG, "[CDC%d] completed cmd 0x%02x plen=%d", itf, par->hdr,
               par->plen);
    }
    break;
  }
  ...
  while (xQueueReceive(s_queues[slot_idx], &msg, 0) == pdTRUE) {
    ...
    if (ev_len > 0) {
      if (msg.type == UART_MSG_TILT) {
        ...
        ESP_LOGD(TAG, "[CDC%d] tilt sensor=%d norns_x=%d norns_y=%d norns_z=%d",
                 itf, msg.data[0], ev[1], norns_y, norns_z);
      } else {
...
static void slot_task(void *arg) {
  ...
  while (1) {
    ...
    while ((avail = tud_cdc_n_available(itf)) > 0) {
      uint8_t buf[64];
      uint32_t n =
          tud_cdc_n_read(itf, buf, avail < sizeof(buf) ? avail : sizeof(buf));
...
void mext_slot_init(void) {
  for (int i = 0; i < MAX_MONOME_SLOTS; i++) {
    ...
    xTaskCreatePinnedToCore(slot_task, name, 4096, (void *)(uintptr_t)i, 5,
                            NULL, MEXT_SLOT_CORE);
  }
}
