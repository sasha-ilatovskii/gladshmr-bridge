...
static const ble_uuid128_t s_svc_uuid = BLE_UUID128_INIT(
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x65, 0x6D, 0x6F, 0x6E, 0x6F, 0x6D);

static const ble_uuid128_t s_data_uuid = BLE_UUID128_INIT(
    0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x65, 0x6D, 0x6F, 0x6E, 0x6F, 0x6D);

static const ble_uuid128_t s_cfg_uuid = BLE_UUID128_INIT(
    0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x65, 0x6D, 0x6F, 0x6E, 0x6F, 0x6D);

static const ble_uuid128_t s_state_uuid = BLE_UUID128_INIT(
    0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x65, 0x6D, 0x6F, 0x6E, 0x6F, 0x6D);
...
static void log_raw_packet_bytes(const uint8_t* buf, uint16_t len)
{
  ...
  {
    int written =
        snprintf(&hex[pos], sizeof(hex) - pos, "%02x%s", buf[i], i + 1 < shown ? " " : "");
    if (written < 0)
    {
      break;
    }
  ...
  ESP_LOGI(TAG,
           "rx raw %s type=0x%02x len=%u%s bytes=%s",
           raw_packet_name(buf[0]),
           buf[0],
           (unsigned) len,
           len > shown ? " truncated" : "",
           hex);
  ...
  if (msg.len > 0)
  {
    if (s_debug_incoming)
    {
      ...
      case UART_MSG_ENC_DELTA:
        ESP_LOGI(
            TAG, "decoded enc_delta n=%u delta=%d", (unsigned) msg.data[0], (int8_t) msg.data[1]);
        break;
      ...
      case UART_MSG_TILT:
        ESP_LOGI(TAG,
                 "decoded tilt sensor=%u x=%d y=%d z=%d",
                 (unsigned) msg.data[0],
                 (int8_t) msg.data[1],
                 (int8_t) msg.data[2],
                 (int8_t) msg.data[3]);
...
// ---------------------------------------------------------------------------
static int data_access_cb(uint16_t conn_handle,
...
// ---------------------------------------------------------------------------
static int state_access_cb(uint16_t conn_handle,
                           uint16_t attr_handle,
                           struct ble_gatt_access_ctxt* ctxt,
                           void* arg)
{
...
// ---------------------------------------------------------------------------
static int cfg_access_cb(uint16_t conn_handle,
                         uint16_t attr_handle,
                         struct ble_gatt_access_ctxt* ctxt,
                         void* arg)
{
...
static int gap_event_cb(struct ble_gap_event* event, void* arg)
{
  ...
  case BLE_GAP_EVENT_SUBSCRIBE:
    if (event->subscribe.attr_handle == s_state_val_handle)
    {
      ...
      ESP_LOGI(TAG, "state notifications %s", s_state_notify_enabled ? "enabled" : "disabled");
  ...
  case BLE_GAP_EVENT_MTU:
    ESP_LOGI(TAG, "MTU negotiated: conn=%d mtu=%d", event->mtu.conn_handle, event->mtu.value);
    break;

  case BLE_GAP_EVENT_PHY_UPDATE_COMPLETE:
    ESP_LOGI(TAG, "PHY update: tx=%d rx=%d", event->phy_updated.tx_phy, event->phy_updated.rx_phy);
    break;
...
static void state_tx_task(void* param)
{
  ...
  while (1)
  {
    ...
    while (batch_len < payload_max && xQueueReceive(s_state_tx_queue, &msg, 0) == pdTRUE)
    {
    ...
    if (!om)
    {
      ...
      {
        ESP_LOGW(TAG, "state notify mbuf alloc failed, count=%" PRIu32, s_state_fail_count);
      }
    ...
    if (rc != 0)
    {
      ...
      if ((s_state_fail_count & 0x3f) == 1)
      {
        ESP_LOGW(TAG, "state notify failed rc=%d count=%" PRIu32, rc, s_state_fail_count);
      }
      vTaskDelay(pdMS_TO_TICKS(2));
    }
  }
...
{
  if (s_conn_handle == BLE_HS_CONN_HANDLE_NONE || !s_state_notify_enabled || !s_state_tx_queue)
  {
    return;
  }
  ...
  if (xQueueSend(s_state_tx_queue, &msg, 0) != pdTRUE)
  {
    ...
    {
      ESP_LOGW(
          TAG, "state notify queue full; dropped old frames count=%" PRIu32, s_state_drop_count);
    }
  }
}
...
void ble_raw_init(void)
{
  ...
  xTaskCreatePinnedToCore(state_tx_task, "ble_state_tx", 4096, NULL, 4, NULL, BLE_APP_CORE);
}
