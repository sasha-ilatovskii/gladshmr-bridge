...
void usb_desc_build(int n_slots)
{
  if (n_slots < 0)
    n_slots = 0;
  if (n_slots > MAX_MONOME_SLOTS)
    n_slots = MAX_MONOME_SLOTS;
  ...
  {
    uint8_t hdr[] = {TUD_CONFIG_DESCRIPTOR(1, itf_count, 0, total_len, 0x80, 100)};
  ...
  for (int i = 0; i < n_slots; i++)
  {
    ...
    uint8_t cdc[] = {TUD_CDC_DESCRIPTOR(itfnum, 0, ep_notif, 8, ep_out, ep_in, 64)};
